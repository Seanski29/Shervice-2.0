...
class _PayrollState extends State<Payroll> {
  ...
  static DateTime _currentPayThursday(DateTime date) {
    ...
    return day.add(Duration(days: daysUntilThursday >= 0 ? daysUntilThursday : daysUntilThursday + 7));
  }
  ...
  Future<Map<String, dynamic>> _getPayrollContext() async {
    ...
    try {
      return await _getJson('$backendUrl/payroll/context?start=$start&end=$end');
    } catch (_) {
      ...
      return {
        'drivers': driversPayload['sample_data_payload'] ?? driversPayload['data'] ?? driversPayload['drivers'],
  ...
  void _syncRouteRates() {
    ...
    final routes = routeRows.values
        .map((row) => _cleanText(row['route_name'] ?? row['destination_name']))
    ...
    for (final route in routes) {
      ...
      final price = _money(_cleanText(routeRow['price'] ?? routeRow['destination_price']));
  ...
  List<Map<String, dynamic>> get _periodTrips {
    return _trips.where((trip) {
      ...
      return date != null && !_dateOnly(date).isBefore(_periodStart) && !_dateOnly(date).isAfter(_payDate);
    }).toList();
  }
  ...
  List<_PayrollRow> get _payrollRows {
    ...
    final driverIds = <String>{
      ..._drivers.map((driver) => _cleanText(driver['driver_id'])).where((id) => id.isNotEmpty),
  ...
  bool _isHalfDay(Map<String, dynamic> row) {
    ...
    final hasMorning = _hasValue(row['morning_in']) || _hasValue(row['morning_out']);
    final hasAfternoon = _hasValue(row['afternoon_in']) || _hasValue(row['afternoon_out']);
      ...
      child: Column(
        ...
        children: [
          ...
          if (_isLoading)
            ...
            _buildMessage('No payroll data found for this pay period.', Icons.payments_outlined, Colors.blueGrey)
          else ...[
                ...
                child: ConstrainedBox(
                  ...
                  child: DataTable(
                    ...
                    columns: const [
                      ...
                      DataColumn(label: Text('Half Day')),
                      DataColumn(label: Text('Absent')),
                      DataColumn(label: Text('Trips')),
                      DataColumn(label: Text('Regular Pay')),
                      DataColumn(label: Text('Deductions')),
                      DataColumn(label: Text('Overtime')),
                      DataColumn(label: Text('Route Pay')),
                    ...
                    rows: rows.map((row) {
                      return DataRow(
                        cells: [
                          ...
                          DataCell(Text('${row.halfDays}')),
                          DataCell(Text('${row.absentDays}')),
                          DataCell(Tooltip(
                            message: row.routeSummary,
                            child: Text('${row.tripCount}'),
                          )),
                          DataCell(Text(_moneyLabel(row.regularPay))),
                          DataCell(Text('-${_moneyLabel(row.totalDeductions)}')),
                          DataCell(Text(_moneyLabel(row.overtimePay))),
                          DataCell(Text(_moneyLabel(row.tripPay))),
                          DataCell(Text(
...
class _PayrollRow {
  ...
  String get routeSummary {
    ...
    return routeBreakdown.entries.map((entry) => '${entry.key}: ${entry.value}').join('\n');
  }
}
