  ...
  Future<void> _fetchStaffDashboardData() async {
    ...
    try {
      final responses = await Future.wait([
        http
            .get(Uri.parse('$backendUrl/schedules/staff/${widget.staffId}'))
            .timeout(const Duration(seconds: 10)),
        http
            .get(Uri.parse('$backendUrl/schedules/blackouts'))
            .timeout(const Duration(seconds: 10)),
      ]);
      final tripsResponse = responses[0];
      final blackoutResponse = responses[1];
  ...
  Future<void> _toggleBlackout(DateTime date) async {
    ...
    if (existing != null) {
      final confirmed = await showEnterpriseDestructiveConfirmation(
        ...
        message:
            'This restores dispatch availability for ${_formatDateOnly(date)}.',
      ...
      try {
        final response = await http.delete(
          Uri.parse(
            '$backendUrl/schedules/blackouts/${existing['blackout_id']}',
          ),
        );
        if (response.statusCode != 200) {
          throw Exception('The blocked date could not be removed.');
        }
        await _fetchStaffDashboardData();
        if (mounted) {
          EnterpriseToasts.success(context, 'Blocked date removed.');
        }
      } catch (error) {
        if (mounted) {
          EnterpriseToasts.error(
            context,
            'Unable to remove blocked date: $error',
          );
        }
      }
      return;
    ...
    return Wrap(
      ...
      children: [
        ...
        SizedBox(
          ...
          height: 44,
          child: TextField(
            key: ValueKey(_searchQuery.isEmpty),
            onChanged: (value) {
              setState(() {
                _searchQuery = value;
              });
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                // Staff schedule list uses the shared row structure.
                GridView.builder(
        ...
        children: [
          ...
          !_isLoading && displayedTrips.isEmpty
              ? EnterpriseEmptyState(
                  ...
                  title: 'No trips match this schedule view',
                  message:
                      'Show the complete schedule to review trips outside the selected date and status.',
    ...
    return AlertDialog(
      ...
      content: _isLoadingOptions
          ? const SizedBox(
              height: 100,
              child: Center(
                child: EnterpriseLoadingIndicator(color: Color(0xFF3B82F6)),
              ),
            )
          : SizedBox(
                ...
                children: [
                  ...
                  if (_vehicles.isEmpty)
                    Container(
                      ...
                      child: const Text(
                        "No vehicles are available for this date.",
                        style: TextStyle(
                          color: Color(0xFFEF4444),
                          fontSize: 13,
                        ),
