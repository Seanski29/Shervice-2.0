...
class SharedDashboardView extends StatefulWidget {
  ...
  const SharedDashboardView({super.key, required this.showClientTrips});
...
class _SharedDashboardViewState extends State<SharedDashboardView> {
    ...
    if (!_isLoading && _metrics.isEmpty) {
      return Padding(
        ...
        child: EnterpriseEmptyState(
          ...
          title: 'Reconnect the operations dashboard',
          message:
              'The operational summary could not be synchronized. Retry to restore live trip, fleet, and workforce data.',
          actionLabel: 'Retry synchronization',
          onAction: _reloadDashboard,
        ),
      );
    }

    final monthlyRows = List.generate(12, (index) {
      return <String, dynamic>{
        'month': _monthNames[index],
        'trips': _monthlyTripTotals[index],
        'maintenance': _monthlyMaintenanceTotals[index],
      };
    });
      ...
      child: ListView(
        ...
        children: [
                      ...
                      SizedBox(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 16),
                            Expanded(
                              child: _buildMaintenanceChartCard(compact: true),
                            ),
                          ],
                        ),
                      ),
          ...
          if (_isLoading && _metrics.isEmpty) ...[
            ...
            EnterpriseDataGrid<DashboardMetric>(
              rows: _metrics,
              rowKey: (metric) => metric.title,
              height: 330,
              showDateRange: false,
              columns: [
                ...
                EnterpriseGridColumn(
                  ...
                  width: 180,
                  value: (metric) =>
                      '${_detailCountForMetric(metric.title)} records',
                ),
              ],
              emptyTitle: 'Synchronize operational measures',
              emptyMessage:
                  'Trip, passenger, driver, vehicle, and maintenance measures will appear after synchronization.',
              emptyActionLabel: 'Retry synchronization',
              onEmptyAction: _reloadDashboard,
              onExportSelection: _exportMetrics,
            ),
            if (widget.showClientTrips) ...[
              const SizedBox(height: 16),
              Text(
              ...
              const SizedBox(height: 8),
              EnterpriseDataGrid<CompanyTripMetric>(
                rows: _companyTrips,
                rowKey: (company) => company.companyName,
                height: 380,
                columns: [
                  EnterpriseGridColumn(
                    label: 'Company',
                  ...
                  EnterpriseGridColumn(
                    label: 'Trips',
                    width: 150,
                    value: (company) => '${company.tripCount}',
                    compare: (first, second) =>
                        first.tripCount.compareTo(second.tripCount),
                  ),
                  EnterpriseGridColumn(
                    label: 'Utilization',
                    width: 170,
                    value: (company) =>
                        '${(company.utilization * 100).toStringAsFixed(1)}%',
                    compare: (first, second) =>
                        first.utilization.compareTo(second.utilization),
                  ),
                ],
                emptyTitle: 'Record the first company trip',
                emptyMessage:
                    'Company trip volume and utilization will appear when dispatch records are available.',
                emptyActionLabel: 'Refresh company activity',
                onEmptyAction: _reloadDashboard,
              ),
              const SizedBox(height: 16),
              Text(
              ...
              EnterpriseDataGrid<Map<String, dynamic>>(
                rows: _topDrivers,
                rowKey: (driver) =>
                    driver['driver_id'] ?? driver['id'] ?? driver.hashCode,
                height: 380,
                columns: [
                  EnterpriseGridColumn(
                    label: 'Driver',
                  ...
                  EnterpriseGridColumn(
                    ...
                    width: 150,
                    value: (driver) =>
                        '⭐ ${_parseScore(driver['rating']).toStringAsFixed(1)}',
                    compare: (first, second) => _parseScore(
                      first['rating'],
                    ).compareTo(_parseScore(second['rating'])),
                  ),
                  EnterpriseGridColumn(
                    label: 'Evaluations',
                    width: 160,
                    value: (driver) => '${_parseInt(driver['review_count'])}',
                  ),
                ],
                emptyTitle: 'Collect the first driver evaluation',
                emptyMessage:
                    'Driver rankings require completed passenger evaluations for the selected period.',
                emptyActionLabel: 'Refresh evaluations',
                onEmptyAction: _reloadDashboard,
              ),
              const SizedBox(height: 16),
              Text(
              ...
              const SizedBox(height: 8),
              EnterpriseDataGrid<MaintenanceAlert>(
                rows: _alerts,
                rowKey: (alert) => alert.vehicleId,
                height: 380,
                columns: [
                  EnterpriseGridColumn(
                    label: 'Vehicle',
                    width: 180,
                    value: (alert) => alert.vehicleId,
                  ),
                  EnterpriseGridColumn(
                    label: 'Required work',
                    width: 420,
                  ...
                  EnterpriseGridColumn(
                    ...
                    width: 170,
                    value: (alert) => '${alert.daysRemaining}',
                    compare: (first, second) =>
                        first.daysRemaining.compareTo(second.daysRemaining),
                    cellBuilder: (context, alert) => Text(
                      '${alert.daysRemaining}',
                      style: TextStyle(
                        color: alert.daysRemaining <= 2
                ...
                emptyTitle: 'No maintenance action is required',
                emptyMessage:
                    'The fleet has no active maintenance alerts. Refresh to check the latest vehicle diagnostics.',
                emptyActionLabel: 'Refresh maintenance queue',
                onEmptyAction: _reloadDashboard,
              ),
            ],
          ],
  ...
  int _detailCountForMetric(String title) {
    final normalized = title.toLowerCase().replaceAll(' ', '');
    for (final entry in _metricDetails.entries) {
      if (entry.key.toLowerCase().replaceAll('_', '').contains(normalized) ||
          normalized.contains(entry.key.toLowerCase().replaceAll('_', ''))) {
  ...
  Future<void> _exportMetrics(List<DashboardMetric> metrics) async {
  ...
  Future<void> _exportMonthlyOperations(List<Map<String, dynamic>> rows) async {
    final buffer = StringBuffer('Month,Trips,Maintenance Events\n');
    for (final row in rows) {
      buffer.writeln('${row['month']},${row['trips']},${row['maintenance']}');
    }
    await _downloadDashboardCsv('shervice-monthly-operations.csv', buffer);
  }

  Future<void> _downloadDashboardCsv(
    String fileName,
    StringBuffer buffer,
  ) async {
    ...
    if (mounted) EnterpriseToasts.success(context, 'Selection exported.');
  }
  ...
  Future<void> _showMetricDetails(DashboardMetric metric) async {
    ...
    if (metric.title == 'Active Drivers' ||
    ...
    try {
      ...
      if (title == 'Active Drivers') {
        return rawItems
            .whereType<Map>()
            .where((item) {
              return (item['employment_status'] ?? 'Active')
                      .toString()
                      .toLowerCase() ==
                  'active';
            })
      ...
      return rawItems
          ...
          .where((item) {
            ...
            return const {
                ...
                (item['driver_id'] == null || item['vehicle_id'] == null);
          })
          .map(
            (item) => {
  ...
  Widget _buildRatingCard({bool compact = false, bool framed = true}) {
    ...
    return Container(
      ...
      child: _isRatingLoading
          ? const SizedBox(
              height: 220,
              child: Center(child: EnterpriseLoadingIndicator()),
            )
          : Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ...
          rankedDrivers.removeWhere((d) {
            final revs = _parseInt(
              d['review_count'] ?? d['total_trips'] ?? d['eval_count'],
            );
            return revs == 0;
          });
          ...
          rankedDrivers.sort((a, b) {
            ...
            final revA = _parseInt(
              a['review_count'] ?? a['total_trips'] ?? a['eval_count'],
            );
            final revB = _parseInt(
              b['review_count'] ?? b['total_trips'] ?? b['eval_count'],
            );
          ...
          setState(() {
            ...
            _ratedDriverCount = serverTotalRated > 0
                ? serverTotalRated
                : actualRatedCount;
          });
        }
  ...
  Future<void> _recomputeDashboardRatingsLocally() async {
    try {
      ...
      const batchSize = 6;
      for (var start = 0; start < rawDrivers.length; start += batchSize) {
        final batch = rawDrivers.skip(start).take(batchSize);
        await Future.wait(
          batch.map((drv) async {
            ...
            try {
              final evalRes = await http
                  .get(Uri.parse('$backendUrl/evaluate/driver/$driverUuid'))
      ...
      if (mounted) {
        setState(() {
          ...
          _averageDriverRating = compiledLeaderboard.isNotEmpty
              ? (globalCumTotal / compiledLeaderboard.length)
              : 0.0;
        });
      }
    } catch (e) {
  ...
  Future<void> _loadMonthlyTripTotals() async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception(
          'Trip summary request failed (${response.statusCode}).',
        );
      }
      ...
      final rawTrips = decoded is Map
          ? (decoded['data'] ??
                decoded['trips'] ??
                decoded['sample_data_payload'])
          : decoded;

      if (rawTrips is! List) {
        throw const FormatException(
          'Trip summary response did not contain a list.',
        );
      }
...
class _DashboardMetadata extends StatelessWidget {
  const _DashboardMetadata({required this.icon, required this.label});
