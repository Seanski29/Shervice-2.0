...
class _StaffTripsState extends State<StaffTrips> {
  ...
  List<Map<String, dynamic>> get _visibleTrips {
    ...
    final rows = _trips.where((trip) {
      ...
      if (_selectedDate != null) {
        ...
        if (_filterMonth != null && tripDate.month != _filterMonth) return false;
      }
      ...
      final haystack = [
        trip['route_name'], trip['driver_name'], trip['plate_number'],
        trip['classification'], trip['ticket_no'], trip['client_company'],
        trip['summary_id'],
      ].join(' ').toLowerCase();
    ...
    rows.sort((a, b) {
      ...
      int cmp = _currentSort == 'Date (Newest)' ? dateB.compareTo(dateA) : dateA.compareTo(dateB);
      if (cmp != 0) return cmp;
  ...
  List<Map<String, dynamic>> _tripsForDate(DateTime date) {
    ...
    return _trips.where((trip) => (trip['schedule_date'] ?? trip['date'] ?? '').toString().startsWith(key)).toList();
  }

  Future<void> _importSummaryFile() async {
    try {
      final files = await FilePicker.pickFiles(type: FileType.custom, allowedExtensions: ['xls', 'xlsx', 'csv']);
      ...
      final request = http.MultipartRequest('POST', Uri.parse('$backendUrl/schedules/upload-summary-xls'));
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded['success'] == true) {
          ...
          if (rows.isEmpty) throw Exception("No valid trip rows found in file.");

          final mappedRows = rows.whereType<Map>().map((e) => Map<String, dynamic>.from(e)).toList();

          if (!mounted) return;
          final createdDate = await Navigator.of(context).push<DateTime>(
            MaterialPageRoute(
              builder: (_) => TripSummaryEditorPage(staffId: widget.staffId, initialRows: mappedRows),
            ),
          );
          ...
          if (createdDate != null && mounted) {
            ...
            if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Imported trips successfully added.')));
          }
        } else {
  ...
  Future<void> _showAddSummaryDialog() async {
    final createdDate = await Navigator.of(context).push<DateTime>(
      MaterialPageRoute(builder: (_) => TripSummaryEditorPage(staffId: widget.staffId)),
    );
    ...
    await _fetchTripSummary();
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Trip summary batch added.')));
  }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _detailRow('Bus Type', (trip['vehicle_type'] ?? trip['bus_type'] ?? '').toString(), isDark),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          SizedBox(
            width: 150,
            child: Text(label, style: TextStyle(color: isDark ? Colors.grey.shade400 : const Color(0xFF64748B), fontWeight: FontWeight.w700)),
          ),
          Expanded(
            child: Text(value.trim().isEmpty ? '-' : value, style: TextStyle(color: isDark ? Colors.white : const Color(0xFF0F172A), fontWeight: FontWeight.w600)),
          ),
        ],
      ),
    );
    ...
    final changed = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => _SummarySheetPage(
          ...
          rows: (summary['rows'] as List? ?? []).whereType<Map>().map((row) => Map<String, dynamic>.from(row)).toList(),
                ...
                return Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text('Select Date', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                    ...
                    GridView.count(
                      ...
                      children: weekdays.map((day) => Center(child: Text(day, style: TextStyle(color: isDark ? Colors.grey.shade400 : const Color(0xFF64748B), fontWeight: FontWeight.w700)))).toList(),
                    ),
                    GridView.builder(
                          ...
                          child: Container(
                            decoration: BoxDecoration(
                              color: isSelected
                                  ...
                                  : hasTrips ? (isDark ? const Color(0xFF0F2D52) : const Color(0xFFEFF6FF)) : Colors.transparent,
                              border: Border.all(
                                color: isToday ? const Color(0xFFF59E0B) : isSelected ? Theme.of(context).colorScheme.primary : borderColor,
                              ),
                              borderRadius: BorderRadius.circular(6),
                            ),
                            child: Column(
                              ...
                              children: [
                                Text(
                                  '$day',
                                  style: TextStyle(
                                    color: isSelected ? Colors.white : hasTrips ? Theme.of(context).colorScheme.primary : Theme.of(context).colorScheme.onSurface,
                                ...
                                if (hasTrips)
                                  FittedBox(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Text('${trips.length} trips', style: TextStyle(color: isSelected ? Colors.white : const Color(0xFF64748B), fontSize: 9, fontWeight: FontWeight.w700)),
                                    ),
                                  ),
                              ],
                            ),
            ...
            if (_isLoading)
              Wrap(
                ...
                children: List.generate(
                  5,
                  (_) => const SizedBox(width: 220, child: EnterpriseSummaryCardSkeleton()),
                ),
              )
            else
  ...
  Widget _buildSummaryCards(bool isDark) {
    ...
    final int completedCount = _visibleTrips.where((trip) {
      final status = (trip['trip_status'] ?? trip['status'] ?? '').toString().toLowerCase();
      return status.contains('completed') || status.contains('done');
    }).length;

    // Colored exactly matching the dashboard reference image style
    final cards = [
      ('Total Trips', '${_visibleTrips.length}', Icons.route_outlined, const Color(0xFF3B82F6)),
      ('Total Passengers', '$_totalPassengers', Icons.groups_outlined, const Color(0xFF8B5CF6)),
      ('Avg Utilization', '${(_averageUtilization * 100).round()}%', Icons.percent, const Color(0xFF06B6D4)),
      ('Completed Trips', '$completedCount', Icons.task_alt_outlined, const Color(0xFF10B981)),
      ('Date Basis', selectedLabel, Icons.calendar_today, const Color(0xFFEF4444)),
    ];
      ...
      children: [
        SizedBox(
          ...
          child: TextField(
            ...
            decoration: _inputDecoration(isDark, 'Search ID, route...', Icons.search),
          ),
        ),
        _dropdown(
          isDark: isDark, width: 140,
          value: _filterMonth == null ? 'All Months' : _monthNames[_filterMonth! - 1],
          values: ['All Months', ..._monthNames],
          onChanged: (value) {
            setState(() {
              if (value == 'All Months') { _filterMonth = null; } else { _filterMonth = _monthNames.indexOf(value!) + 1; _selectedDate = null; }
              _resetPagination();
            });
          },
        ),
        _dropdown(
          isDark: isDark, width: 120,
          value: _filterYear == null ? 'All Years' : _filterYear.toString(),
          values: ['All Years', ..._availableYears.map((y) => y.toString())],
          onChanged: (value) {
            setState(() {
              if (value == 'All Years') { _filterYear = null; } else { _filterYear = int.parse(value!); _selectedDate = null; }
              _resetPagination();
            });
          },
        ),
        _dropdown(
          isDark: isDark, width: 150,
          value: _currentSort, values: ['Date (Newest)', 'Date (Oldest)'],
          onChanged: (value) => setState(() { _currentSort = value ?? 'Date (Newest)'; _resetPagination(); }),
        ),
        _dropdown(
          isDark: isDark, width: 160,
          value: _driverOptions.contains(_selectedDriver) ? _selectedDriver : 'All Drivers',
          values: _driverOptions,
          onChanged: (value) => setState(() { _selectedDriver = value ?? 'All Drivers'; _resetPagination(); }),
        ),
        _dropdown(
          isDark: isDark, width: 160,
          value: _vehicleOptions.contains(_selectedVehicle) ? _selectedVehicle : 'All Vehicles',
          values: _vehicleOptions,
          onChanged: (value) => setState(() { _selectedVehicle = value ?? 'All Vehicles'; _resetPagination(); }),
        ),
      ],
    );
  }
  ...
  Widget _dropdown({required bool isDark, required double width, required String value, required List<String> values, required ValueChanged<String?> onChanged}) {
    final safeValue = values.contains(value) ? value : values.first;
    return Container(
      width: width, height: 42, padding: const EdgeInsets.symmetric(horizontal: 12),
      decoration: BoxDecoration(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          value: safeValue, isExpanded: true, dropdownColor: Theme.of(context).cardColor,
          style: TextStyle(color: Theme.of(context).colorScheme.onSurface, fontSize: 13, fontWeight: FontWeight.w600),
          items: values.toSet().map((item) => DropdownMenuItem(value: item, child: Text(item))).toList(),
          onChanged: onChanged,
        ),
      ),
    );
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Text(
            label,
            style: TextStyle(
              color: selected ? Colors.white : (isDark ? Colors.grey.shade400 : const Color(0xFF475569)),
              fontWeight: FontWeight.w800,
            ),
          ),
        ),
  ...
  Widget _buildSummaryTable(bool isDark) {
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Chip(label: Text('${rows.length} records', style: const TextStyle(fontWeight: FontWeight.bold))),
            const SizedBox(width: 12),
            OutlinedButton.icon(
        ...
        EnterpriseDataGrid<Map<String, dynamic>>(
          ...
          columns: [
            EnterpriseGridColumn(label: 'Date', width: 110, value: (trip) => (trip['schedule_date'] ?? trip['date'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Summary ID', width: 170, value: (trip) => (trip['summary_id'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Day', width: 105, value: _workingDay),
            EnterpriseGridColumn(label: 'Type', width: 105, value: (trip) => (trip['vehicle_type'] ?? trip['bus_type'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Class', width: 110, value: (trip) => (trip['classification'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Plate', width: 110, value: (trip) => (trip['plate_number'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Capacity', width: 95, value: (trip) => (trip['seating_capacity'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Ticket', width: 105, value: (trip) => (trip['ticket_no'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Driver', width: 170, value: (trip) => (trip['driver_name'] ?? 'Unassigned').toString()),
            EnterpriseGridColumn(label: 'Route', width: 190, value: (trip) => (trip['route_name'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Pax', width: 75, value: (trip) => (trip['passenger_count'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Depart', width: 95, value: (trip) => _timeText(trip['departure_time'])),
            EnterpriseGridColumn(label: 'Arrival', width: 95, value: (trip) => _timeText(trip['estimated_arrival_time'])),
            EnterpriseGridColumn(label: 'Util', width: 80, value: _utilizationText),
            EnterpriseGridColumn(label: 'Remarks', width: 180, value: (trip) => (trip['remarks'] ?? '').toString()),
          ...
          emptyMessage: 'Clear the current date and status filters to restore the trip ledger.',
        ),
      ],
    );
  }
  ...
  Widget _buildEditableSummarySheet(bool isDark) {
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Chip(label: Text('${groups.length} batches', style: const TextStyle(fontWeight: FontWeight.bold))),
            const SizedBox(width: 12),
            OutlinedButton.icon(
              onPressed: () => _openCalendarModal(isDark),
              icon: const Icon(Icons.calendar_month, size: 16),
        ...
        EnterpriseDataGrid<Map<String, dynamic>>(
          ...
          loading: _isLoading,
          height: 560,
          rowKey: (summary) => Object.hash(summary['summary_id'], summary['date']),
          columns: [
            EnterpriseGridColumn(label: 'Summary ID', width: 180, value: (summary) => (summary['summary_id'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Date', width: 120, value: (summary) => (summary['date'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Company', width: 190, value: (summary) => (summary['company'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Day', width: 120, value: (summary) => (summary['working_day'] ?? '').toString()),
            EnterpriseGridColumn(label: 'Trips', width: 85, value: (summary) => '${summary['row_count'] ?? 0}'),
            EnterpriseGridColumn(label: 'Passengers', width: 105, value: (summary) => '${summary['passenger_count'] ?? 0}'),
            EnterpriseGridColumn(label: 'Vehicles', width: 260, value: (summary) => (summary['vehicles'] ?? '').toString()),
            EnterpriseGridColumn(
              label: 'Action',
              width: 110,
              value: (_) => 'Open',
          ...
          emptyMessage: 'Clear the selected reporting period to restore summary batches.',
        ),
      ],
    );
  }
...
class _SummarySheetPage extends StatelessWidget {
  ...
  String _generateCSV() {
    const headers = [
      'No.', 'Date', 'Working Day', '(Bus) Type', 'Classification', 'Bus No. (Jeep plate no.)',
      'Seating Capacity', 'Ticket no.', 'Driver', 'Route', 'No. of Passengers',
      'Dept. Time', 'Arrival Time', 'Rate per Trip', 'Cost/head/day', 'Utilization Rate', 'REMARKS',
    ];
  ...
  Future<void> _exportSummary(BuildContext context) async {
    try {
      ...
      final Uri? outputFile = await FilePicker.saveFile(
        dialogTitle: 'Export Summary', fileName: '${safeSummaryId}_Export.csv',
        type: FileType.custom, allowedExtensions: ['csv'], bytes: fileBytes,
      );
      if (outputFile == null) return;
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Export successfully triggered!')));
    } catch (e) {
  ...
  Future<void> _exportXlsx(BuildContext context) async {
    try {
      ...
      final outputFile = await FilePicker.saveFile(
        dialogTitle: 'Export Summary as Excel', fileName: '${safeSummaryId}_Export.xlsx',
        type: FileType.custom, allowedExtensions: ['xlsx'], bytes: _generateXlsx(),
      );
      if (outputFile != null && context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Excel export completed.')));
    } catch (e) {
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Excel export failed: $e')));
    }
  }
          ...
          Padding(
            ...
            child: OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(
                foregroundColor: isDark ? Colors.blue.shade300 : const Color(0xFF2563EB),
                  ...
                  final changed = await Navigator.of(context).push<bool>(
                    MaterialPageRoute(
                      builder: (_) => TripSummaryEditorPage(
                        staffId: staffId, summaryId: summaryId,
                        initialRows: rows, onSaveRow: onSaveRow,
                      ),
                    ),
                  );
            ...
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    Expanded(
                      child: Text('Summary Information', style: TextStyle(color: isDark ? Colors.white : const Color(0xFF0F172A), fontWeight: FontWeight.w800, fontSize: 18)),
                    ),
                    Text('${rows.length} trips', style: TextStyle(color: isDark ? Colors.grey.shade400 : const Color(0xFF64748B), fontWeight: FontWeight.w800)),
                  ],
                ),
              ),
              Expanded(
                child: Scrollbar(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: SingleChildScrollView(
                      child: DataTable(
                        headingRowColor: WidgetStatePropertyAll(isDark ? const Color(0xFF0F172A) : const Color(0xFFF8FAFC)),
                        columns: const [
                          DataColumn(label: Text('No.')), DataColumn(label: Text('Date')),
                          DataColumn(label: Text('Day')), DataColumn(label: Text('Type')),
                          DataColumn(label: Text('Class')), DataColumn(label: Text('Plate')),
                          DataColumn(label: Text('Capacity')), DataColumn(label: Text('Ticket')),
                          DataColumn(label: Text('Driver')), DataColumn(label: Text('Route')),
                          DataColumn(label: Text('Pax')), DataColumn(label: Text('Depart')),
                          DataColumn(label: Text('Arrival')), DataColumn(label: Text('Util')),
                        ...
                        rows: List.generate(rows.length, (index) {
                          ...
                          return DataRow(
                            cells: [
                              ...
                              DataCell(_plainCell((trip['vehicle_type'] ?? trip['bus_type'] ?? '').toString(), 180)),
  ...
  Widget _plainCell(String value, double width) {
    return SizedBox(width: width, child: Text(value.trim().isEmpty ? '-' : value, maxLines: 2, overflow: TextOverflow.ellipsis));
  }
